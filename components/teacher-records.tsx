...
export function TeacherRecords({ rows }: { rows: AttemptRow[] }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => normalizeKeyword(row.class_name).includes(normalizedKeyword));
      ...
      <div className="border-b border-[#eee0d4] px-5 py-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#7a7168]">点击每轮录音即可回听；评分为任务型学习反馈，不是专业语音测评。</p>
          </div>
          <p aria-live="polite" className="rounded-full bg-[#edf6f8] px-3 py-1.5 text-sm font-bold text-[#315f6b]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1380px] border-collapse text-left text-sm">
          ...
          <tbody>{filteredRows.map((row) => {
            ...
            return <tr key={row.id} className="border-t border-[#f0e4da] align-top hover:bg-[#fffbf7]"><td className="whitespace-nowrap px-4 py-4 text-[#687168]">{new Date(row.submitted_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}</td><td className="px-4 py-4"><p className="font-bold">{row.student_name}</p><p className="text-xs text-[#7a7168]">{row.student_id}</p></td><td className="px-4 py-4">{row.class_name}</td><td className="px-4 py-4 font-bold text-[#416b36]">{row.scene_title}</td><td className="px-4 py-4 text-xl font-black text-[#d94f08]">{row.total_score ?? '—'}</td><td className="whitespace-nowrap px-4 py-4 leading-6">任务 {row.task_score ?? '—'}/40<br />句型 {row.sentence_score ?? '—'}/30<br />清晰 {row.clarity_score ?? '—'}/20<br />话轮 {row.interaction_score ?? '—'}/10</td><td className="min-w-72 px-4 py-4"><div className="space-y-2">{clips.length ? clips.map((clip, index) => <div key={clip.key} className="flex items-center gap-2"><span className="w-12 text-xs font-bold">第{clip.round}轮</span><audio controls preload="none" src={`/api/audio?attempt=${encodeURIComponent(row.id)}&clip=${index}`} className="h-9 w-52" /></div>) : <span className="text-[#8a837c]">无录音</span>}</div></td><td className="max-w-md whitespace-pre-line px-4 py-4 leading-6 text-[#59645a]">{row.transcript}</td><td className="max-w-sm px-4 py-4 leading-6 text-[#59645a]">{row.feedback || '—'}</td></tr>;
          })}</tbody>
        </table>
