...
export function WordStudent() {
  const queryCode = new URLSearchParams(window.location.search).get('unit') || '';
  ...
  const [completed, setCompleted] = useState<{ averageScore: number; averageSelf: number; wordCount: number } | null>(null);
  ...
  async function loadUnit(nextCode = code) {
    ...
    try {
      const payload = await wordRequest<{ ok: true; unit: WordUnit }>('studentGetUnit', { shareCode: normalized });
      setUnit(payload.unit); setCode(normalized);
      const url = new URL(window.location.href); url.searchParams.set('unit', normalized); window.history.replaceState({}, '', url);
    } catch (requestError) {
      setUnit(null); setError(requestError instanceof Error ? requestError.message : '没有找到这个单元。');
  ...
  async function startPractice(event: SyntheticEvent<HTMLFormElement>) {
    ...
    setLoading(true); setError('');
    try {
      const payload = await wordRequest<{ attemptId: string; submitToken: string }>('studentStart', {
        shareCode: unit.shareCode, studentName: name, className, recordingConsent: consent,
      });
      setAttemptId(payload.attemptId); setSubmitToken(payload.submitToken);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '暂时无法开始。'); }
    finally { setLoading(false); }
  }

  function resetClip() {
    if (clipUrl) URL.revokeObjectURL(clipUrl);
    setClip(null); setClipUrl(''); setTranscript(''); setConfidence(0); setSelfRating(0); setFeedback(null);
  }
  ...
  async function beginRecording() {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        setClip(blob); setClipUrl(URL.createObjectURL(blob));
      ...
      if (recognition) {
        ...
        let bestConfidence = 0; const heard: string[] = [];
        recognition.onresult = (event) => {
          for (let resultIndex = event.resultIndex; resultIndex < event.results.length; resultIndex += 1) {
            const result = event.results[resultIndex];
            if (result.isFinal && result[0]) { heard.push(result[0].transcript); bestConfidence = Math.max(bestConfidence, result[0].confidence || 0); }
          }
          setTranscript(heard.join(' ').trim()); setConfidence(bestConfidence);
        };
  ...
  async function nextWord() {
    if (!unit) return;
    if (index < unit.words.length - 1) { setIndex((value) => value + 1); resetClip(); return; }
    setSaving(true); setError('');
    try {
      const result = await wordRequest<{ averageScore: number; averageSelf: number; wordCount: number }>('studentComplete', { attemptId, submitToken });
      setCompleted(result);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '提交失败，请重试。'); }
    finally { setSaving(false); }
  }
  ...
  if (!unit) return (
    <main className="word-shell grid min-h-screen place-items-center px-5 py-10">
      <section className="word-card w-full max-w-xl p-7 sm:p-10">
        <div className="word-mark"><Volume2 /></div>
  ...
  if (!attemptId) return (
    <main className="word-shell min-h-screen px-5 py-8">
      <section className="mx-auto max-w-3xl">
        <div className="mb-5 flex items-center justify-between gap-3"><div><p className="word-kicker">{unit.teacherName} · {unit.shareCode}</p><h1 className="mt-1 text-3xl font-black">{unit.title}</h1></div><span className="word-pill">{unit.words.length} 个单词</span></div>
        <div className="word-card p-6 sm:p-8">
          <div className="rounded-2xl bg-cyan-50 p-4 text-sm leading-6 text-cyan-950"><strong>练习步骤：</strong>先听真人示范 → 点击录音并手动结束 → 回听自己的录音 → 自评 → 查看系统反馈。<span className="mt-1 block">建议使用 Chrome 或 Edge；如果学习通内无法自动识别，请点右上角“在浏览器打开”。录音仍可正常上传给老师回听。</span>{unit.note && <span className="block mt-1">老师提示：{unit.note}</span>}</div>
          <form onSubmit={startPractice} className="mt-6 grid gap-5">
            <div className="grid gap-4 sm:grid-cols-2"><label className="word-label">姓名<input required value={name} onChange={(event) => setName(event.target.value)} className="word-input mt-2" placeholder="请输入真实姓名" /></label><label className="word-label">班级<input required value={className} onChange={(event) => setClassName(event.target.value)} className="word-input mt-2" placeholder="如：城轨信号2401" /></label></div>
            <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6"><input required type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-1 h-5 w-5 accent-indigo-600" /><span>我同意将本次练习录音、自动识别文字、评分和自评结果上传给任课教师，用于学习反馈。</span></label>
            {error && <p role="alert" className="word-error">{error}</p>}
            <button disabled={loading || !name.trim() || !className.trim() || !consent} className="word-primary w-full">{loading ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <><Headphones className="h-5 w-5" />开始跟读</>}</button>
          </form>
        </div>
      </section>
    </main>
  ...
  if (completed) return (
    <main className="word-shell grid min-h-screen place-items-center px-5 py-10"><section className="word-card w-full max-w-2xl p-8 text-center sm:p-12"><CheckCircle2 className="mx-auto h-16 w-16 text-emerald-500" /><p className="word-kicker mt-5">练习已上传</p><h1 className="mt-2 text-4xl font-black">完成得很认真！</h1><div className="mt-7 grid gap-3 sm:grid-cols-3"><div className="word-stat"><b>{completed.wordCount}</b><span>完成单词</span></div><div className="word-stat"><b>{completed.averageScore}</b><span>系统练习分</span></div><div className="word-stat"><b>{completed.averageSelf}/3</b><span>平均自评</span></div></div><p className="mt-6 text-sm leading-6 text-slate-500">系统分用于判断目标单词是否被识别，不等同于专业音素测评。老师可以回听你的录音。</p><button onClick={() => window.location.reload()} className="word-primary mt-7">再练一次</button></section></main>
  );
  ...
  return (
    <main className="word-shell min-h-screen px-4 py-5 sm:px-6 sm:py-8">
      <section className="mx-auto max-w-4xl">
        <div className="mb-4 flex items-center justify-between gap-4"><div><p className="word-kicker">{unit.title}</p><p className="mt-1 text-sm font-bold text-slate-500">{name} · {className}</p></div><span className="word-pill">{index + 1} / {unit.words.length}</span></div>
        <div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-r from-indigo-600 to-cyan-400 transition-all" style={{ width: `${((index + 1) / unit.words.length) * 100}%` }} /></div>
        <article className="word-card mt-5 overflow-hidden">
          ...
          <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_1.15fr]">
            <section><p className="word-step">STEP 1 · 听真人示范</p>{current.audioUrl ? <audio controls preload="metadata" src={current.audioUrl} className="mt-4 w-full" /> : <p className="word-error mt-4">示范音频暂时无法加载，请刷新页面。</p>}<div className="mt-5 rounded-2xl bg-indigo-50 p-4 text-sm leading-6 text-indigo-950"><Sparkles className="mb-2 h-5 w-5 text-indigo-600" />先只听一遍，注意重音和音节；第二遍再轻声模仿。</div></section>
